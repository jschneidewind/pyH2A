...
from input_modification import convert_input_to_dictionary, process_input, process_table, insert, read_textfile, set_by_path, execute_plugin
...
def MACRS_depreciation(plant_years, depreciation_length, annual_depreciable_capital):
	...
	original_macrs = read_textfile('../Lookup_Tables/MACRS.csv', delimiter = '	')
...
def discounted_cash_flow_function(inp, values, parameters, attribute = 'h2_cost', 
	...
	'''Wrapper function for Discounted_Cash_Flow, substituting provided values at specified parameter
	positions and returning desired attribute of Discounted_Cash_Flow object.
	
	If inp is a string of a file_name, the provided file is converted to input dictionary inp.
	If inp is already a dictionary, it is used as such.

	values is a 1 or 2D array containing the value which are to be used.
	Parameters is a 1 or 2D array containing the parameter specifications (location within inp 
	[top_key, middle_key, bottom_key])

	For each value (1D array) or set of values (2D array), the values are substituted in inp, 
	Discounted_Cash_Flow (dcf) is executed and the dcf object is generated.

	attribute specifies the attribute of dcf which is to be returned. If attribube is 'plugs',
	the dcf.plugs dictionary is retrieved, containing all plugin objects. plugin is a string 
	specifying th desired plugin and plugin_attr specifies the attribute of this plugin, which is to
	be returned.
	'''
...
class Discounted_Cash_Flow:
	...
	Name | Type | Position
	--- | --- | ---
	initial_equity_depreciable_capital | function | num
	non_depreciable_capital_costs | function | num
	replacement_costs | function | num
	fixed_operating_costs | function | num
	variable_operating_costs | function | num

	Optional:

	Inclusion of plugins via

	plugin_name | plugin | num
	...
	def workflow(self, inp, npv_dict, plugs_dict):
		...
		for key in sorted_keys:
			if inp['Workflow'][key]['Type'] == 'function':
				...
				execute_plugin(key, plugs_dict, print_info = self.print_info, self = self)
	...
	def inflation(self):
		...
		plant_cost = read_textfile('../Lookup_Tables/Plant_Cost_Index.csv', delimiter = '	')
		gdp_deflator_price = read_textfile('../Lookup_Tables/GDP_Implicit_Deflator_Price_Index.csv', delimiter = '	')
		labor_price = read_textfile('../Lookup_Tables/Labor_Index.csv', delimiter = '	')
		chemical_price = read_textfile('../Lookup_Tables/SRI_Chemical_Price_Index.csv', delimiter = '	')
	...
	def cash_flow(self):
		...
		npv_after_tax_post_depreciation = np.npv(self.after_tax_nominal_irr, after_tax_post_depreciation_cash_flow)
	...
	def check_processing(self):
		...
		exceptions = ['Workflow', 'Display Parameters', 'Analyss - Sensitivity_Analyss']
